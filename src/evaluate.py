...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Set CUDA device if needed
...
def main(args): 
    ...
    if args.vis_save_path is not None:
        logger.info(f"Saving visualizations to {args.vis_save_path}...")
        os.makedirs(args.vis_save_path, exist_ok=True)
        # Save the visualizations

    visualize_forecasts(
        results["context_sequences"][:5],
        results["true_sequences"][:5],
        results["forecast_sequences"][:5],
        features_names=["Prey", "Predator"],
        num_samples=5, # we only visualize 5 samples
